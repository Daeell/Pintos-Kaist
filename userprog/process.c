...
load(const char *file_name, struct intr_frame *if_)
{
	...
	file = filesys_open(file_name); // 위에서 process_activate로 active하였기 때문에 이제 실행가능한 파일이 되었고 이를 open.
	if (file == NULL)
	{
	...
	for (i = 0; i < ehdr.e_phnum; i++)
	{
		...
		if (file_ofs < 0 || file_ofs > file_length(file))
			/* file_ofs가 file_length보다 크다 == 더이상 쓸 공간이 없다 혹은 0보다 작다 == 파일에 문제가 있다 */
			goto done;
		file_seek(file, file_ofs); /* 커서를 file_ofs로 이동한다 -> 그 후에도 사용하기 때문*/

		if (file_read(file, &phdr, sizeof phdr) != sizeof phdr)
			/* file을 읽어서 phdr에 복사한다. file를 읽은 사이즈가 filer과 같은지 검사한다.*/
			goto done;
		file_ofs += sizeof phdr;
		/* file_ofs의 커서를 phdr만큼 이동한다 -> phdr만큼 읽었으니까 다음 부분을 읽으려고*/
		switch (phdr.p_type)
		{
		case PT_NULL:
		case PT_NOTE:
		...
		case PT_LOAD:
			...
			/* phdr이 file안에 있는 valid하면서 load가능한 segment한 내용을 담고 있는지 확인한다. */
			{
